...
def build_tile_base(tiles_dir, tile_height):
    #יש רשימה אחת גדולה שלכל תמונה בה יש רשימה משלה ןבתוכה יש רשימות
    #  כך שכל רשימה שלה היא שורה בתמונה ובכל שורה כזאת
    #  יש  טפלס כך שכל טפל הוא פיקסל(עמודה)
    # ככל שמספר הגובה(השורות) של תמונה גבוהה יותר כך יש גם יותר עמודות(פיקסלים) לכל שורה
...
def load_image(image_filename):
    ...
    (and each list contains tuples of 3 RGB) """
